...
export class DashboardAdminService {
  static async getDashboardInfo(): Promise<IDashboardAdminInfo> {
    if (USE_MOCK) {
      ...
      // Filtros de data (Simulando Março como mês atual)
      ...
      const percentualCrescimento = totalVendasAnterior > 0 
      ...
      const statusData = statusLabels.map(label => 
        pedidos.filter(p => p.status === label).length
      );

      // Agrupamento por Categoria (simplificado para os meses Jan, Fev, Mar)
      ...
      const atividadesRecentes: IAtividadeRecente[] = pedidos.slice(-5).reverse().map(p => ({
        uuid: p.uuid,
        tipo: 'Venda',
