/**
 * Testes E2E de Fluxo de Recuperação com Atualização de Endereço via UI Real
 ...
 * - Cliente acessa "Meu Perfil" → cadastrar novo endereço via UI
 * - Cliente seleciona novo endereço no pedido via UI
...
describe('Fluxo Cliente - Atualização de Endereço em Falha de Entrega (UI Real)', () => {
  ...
  beforeEach(() => {
    ...
    cy.marcarFalhaEntregaApi(vendaUuid, 'Endereço incompleto - número não encontrado');
  ...
  describe('Acesso ao Pedido com Falha via UI', () => {
    it('deve permitir cliente acessar pedido com falha via UI', () => {
    ...
    it('deve exibir status de falha no pedido via UI', () => {
  ...
  describe('Cadastro de Novo Endereço via UI', () => {
    beforeEach(() => {
      cy.loginApi(emailCliente, senhaCliente);
    });

    it('deve permitir cliente cadastrar novo endereço via UI', () => {
    ...
    it('deve impedir cadastro de endereço incompleto via UI', () => {
      cy.visit('/minha-conta');
  ...
  describe('Associação de Novo Endereço ao Pedido via UI', () => {
    beforeEach(() => {
      cy.loginApi(emailCliente, senhaCliente);
    ...
    it('deve permitir cliente associar novo endereço ao pedido via UI', () => {
      cy.visit('/pedidos');
      cy.get('[data-cy="loading"]', { timeout: 10000 }).should('not.exist');
  ...
  describe('Fluxo Completo de Recuperação - E2E UI Real', () => {
    it('deve executar fluxo completo: cliente cadastra endereço → admin redespacha → entrega com sucesso', () => {
      // 1. Cliente cadastra novo endereço via UI
      cy.loginApi(emailCliente, senhaCliente);
      cy.visit('/minha-conta');
      ...
      cy.request({
        ...
        // 2. Associar novo endereço ao pedido via API (simulando ação na UI)
        cy.request({
          method: 'PUT',
      ...
      // 3. Admin redespacha pedido via UI
      cy.loginAdminApi();
      ...
      // 4. Admin confirma nova entrega via UI
      cy.contains(vendaUuid.split('-')[1].toUpperCase())
        .parents('tr')
      ...
      // 6. Cliente confirma recebimento via UI
      cy.loginApi(emailCliente, senhaCliente);
      cy.visit('/pedidos');
      cy.get('[data-cy="loading"]', { timeout: 10000 }).should('not.exist');
